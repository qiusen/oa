...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/managerRole/managerRoleAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//managerRole/managerRoleAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/managerRole/managerRoleAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/managerRole/managerRoleAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.email + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/managerRole/managerRoleAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="managerRoleForm" id="managerRoleForm" method="post" action="managerRoleAction.${actionExt}" >
...
<div id="searchbar">
邮箱：<input id="managerRole.email" type="text" name="managerRole.email" value="${managerRole.email }"/>
