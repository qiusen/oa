...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/menu/menuAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//menu/menuAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/menu/menuAction!add.${actionExt}";
	}
	if(item.value=='edit'){
...
<form name="menuForm" id="menuForm" method="post" action="menuAction.${actionExt}" >
...
<div id="searchbar">
菜单名称：<input id="menu.menuname" type="text" name="menu.menuname" value="${menu.menuname }"/>
