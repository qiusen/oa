...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '目录名称', name: 'catalogname',type: 'text' , width: '15%'},
		...
		{ display: '所属菜单', name: 'menuId', width: '15%' ,render: function (rowdata, rowindex, value)
        {
        	var r = "-";
        	<c:forEach items="${menuList }" var="menu">
        	if(rowdata.menuId==${menu.id}){
                r = "${menu.menuname}";
            }
            </c:forEach>
            return r;
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/catalog/catalogAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//catalog/catalogAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/catalog/catalogAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { alert('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/catalog/catalogAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		var row = manager.getSelectedRow();
        if (!row) { alert('请选择行'); return; }
		$.ligerDialog.confirm('确认删除模块 ' + row.catalogname + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/catalog/catalogAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="catalogForm" id="catalogForm" method="post" action="catalogAction.${actionExt}" >
...
<div id="searchbar">
目录名称：<input id="catalog.catalogname" type="text" name="catalog.catalogname" value="${catalog.catalogname }"/>
